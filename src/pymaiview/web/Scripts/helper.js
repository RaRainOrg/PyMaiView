...
const baseURL = globalThis.__PYMAIVIEW_SKIN__ || './Skin/', baseImageKeys = [
    'no_image',
    ...
    'judge_text_normal', 'judge_text_break', 'judge_text_miss',
];
...
export async function videoRender(audioManager, canvas, renderer, {
    ...
    introDuration = includeIntro ? Math.max(0, Number(introDuration) || INTRO_TIMELINE.total) : 0;

    const outlineImage = await (async () => {
        try {
            const response = await fetch((globalThis.__PYMAIVIEW_SKIN__ || './Skin/') + 'outline.png');
    ...
    try {
        ...
        if (includeAudio) {
            ...
            if (slicedAudio) {
                ...
                if (introDuration > 0) {
                    ...
                    // Play track_start when the opening transition completes
                    // and the chart introduction begins (00:03.000).
                    ...
                    if (includeSfx && trackStartBuf && slicedAudio) {
                        ...
                        const introSfxStartSec = Math.min(
                            introDuration,
                            INTRO_TIMELINE.transition,
                        );
                        const startSample = Math.max(0, Math.floor(introSfxStartSec * sr));
        ...
        if (typeof globalThis.__PYMAIVIEW_DOWNLOAD__ !== 'function') {
            throw new Error('缺少 pymaiview 输出接收器');
        }
        await globalThis.__PYMAIVIEW_DOWNLOAD__(blob, mime, ext);
